...
export class ECONode {
  ...
  _getLevel() {
    if (this.nodeParent && this.nodeParent.id == -1) {
      return 0;
    } else return this.nodeParent && this.nodeParent._getLevel() + 1;
  }
  ...
  _getChildrenCenter(tree: any) {
    const node: any = this._getFirstChild();
    const node1: any = this._getLastChild();
    return (
...
*/
  ...
  add(idMain:any,designationName: any,id: any, pid: any, dsc: any, w: any, h: any, c: any, bc: any, lc: any, meta: any) {
    ...
    let pnode: any = null; //Search for parent node in database
    if (pid == -1) {
      pnode = this.root;
    } else {
      for (let k = 0; k < this.nDatabaseNodes.length; k++) {
