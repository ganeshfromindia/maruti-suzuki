  ...
  <div class="wrapper">
    ...
    <ng-container *ngFor="let node of tree.nDatabaseNodes;let i=index">
      <div *ngIf="!node._isAncestorCollapsed()" class="item"
        [ngStyle]="{left:node.XPosition+'px',top:node.YPosition+'px',width:node.w+'px',height:node.h+'px'}">
...
export class TreeViewComponent  {
  ...
  addNodes(tree:ECOTree,node:any,parent:any=null)
  {
    ...
    node.id=tree.nDatabaseNodes.length
      tree.add(node.idMain,node.designationName,node.id,parent.id,node.title, node.width, node.height, node.color, node.background, node.linkColor, node.data)
      if (node.children)
      {
